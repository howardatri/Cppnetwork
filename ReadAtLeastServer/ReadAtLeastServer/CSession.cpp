...
void CSession::Send(char* msg, int max_length) {
	...
	if (send_que_size > 1000) {
		cout << "send queue size is too large, size is " << send_que_size << endl;
		return;
	}
	if (_send_que.size() > 0) {
		pending = true;
...
void CSession::HandleReadHead(const boost::system::error_code& error, size_t  bytes_transferred,
	...
	if (!error) {
		...
		cout << "head data_len is " << data_len << endl;
		...
		boost::asio::async_read(_socket, boost::asio::buffer(_recv_msg_node->_data,
			_recv_msg_node->_total_len), std::bind(&CSession::HandleReadMsg, this,
				std::placeholders::_1,std::placeholders::_2,SharedSelf()));
	}
	else {
...
void CSession::PrintRecvData(char* data, int length) {
	...
	cout << "receive data is " << result << endl;
}
...
void CSession::HandleReadMsg(const boost::system::error_code& error, size_t  bytes_transferred,
	...
	if (!error) {
		...
		std::cout << "handle read msg failed, error is " << error.what() << endl;
		Close();
		_server->ClearSession(_uuid);
